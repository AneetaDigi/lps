...
define("P_SUBSCRIBE_PLUGIN_VERSION", "1.5.2");
...
function sfba_activation_redirect( $plugin ) {
    ...
    if( $plugin == plugin_basename( __FILE__ ) ) {
        $is_shown = get_option("sfba_subscribe_form_update_message");
        if($is_shown === false) {
            wp_redirect(admin_url("edit.php?post_type=sfba_subscribe_form&type=update"));
        } else {
...
if ( !function_exists( 'subscribe_form_send_message_to_owner') ) {
	function subscribe_form_send_message_to_owner() {
		...
		if(empty($errorArray)) {
			...
			$subject = "Subscribe Forms: ".$domain;
